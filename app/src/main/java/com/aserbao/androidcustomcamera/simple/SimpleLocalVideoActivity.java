...
public class SimpleLocalVideoActivity extends AppCompatActivity implements MediaPlayerWrapper.IMediaCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_simple_local_video);
...
//        progressDialog = DialogManager.showProgressDialog(mContext);
            ...
            public void onSuccess() {
                ...
                if (!isLocalPortrait) {
                    if (!TextUtils.isEmpty(mSavevideotemp)) {
                        ...
                        VideoPlayerActivity2.launch(SimpleLocalVideoActivity.this,mSavevideotemp);
                    } else {
                        VideoPlayerActivity2.launch(SimpleLocalVideoActivity.this,mOutVideoPath);
                    }
                    myHandler.sendEmptyMessage(CLIPPER_GONE);
                } else {
                    translateVideo();
    ...
    private static class MyHandler extends Handler {
        private WeakReference<SimpleLocalVideoActivity> mWeakReference;

        public MyHandler(SimpleLocalVideoActivity localVideoActivity) {
            mWeakReference = new WeakReference<SimpleLocalVideoActivity>(localVideoActivity);
        }

        @Override
        public void handleMessage(Message msg) {
            final SimpleLocalVideoActivity localVideoActivity = mWeakReference.get();
